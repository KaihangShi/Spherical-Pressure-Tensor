...
!   - z-axis is always normal to the planar interface
...
! ---------------- Input Parameters ---------------------------!
! Control parameters 
Integer, Parameter ::  st_frame=119521
Integer, Parameter ::  nd_frame=120000

! xtc file PATH 
Character*256, Parameter :: xtcpath = "/gpfs_backup/gubbins_data/kshi3/sphe_pressure_tensor/&
                                      &new_traj_462023/ice_ih_prismaticII_1bar/traj_comp_pSec.xtc"  
! solid cluster center-of-mass file (evaluated using oxygen atom only)
Character*256, Parameter :: compath = "/gpfs_backup/gubbins_data/kshi3/sphe_pressure_tensor/&
                                      &new_traj_462023/ice_ih_prismaticII_1bar/CM_coordinates_pSec.dat"
...
Do iframe = 1, ns_frame
  ...
  ! assuming z-direction is perpendicular to the planar interface
  area = boxsz(1,iframe)*boxsz(2,iframe)   ! Lx  * Ly
  ...
  Do imol = 1, n_mol_tot
    ...
    if (dABS(rz(imol,iframe)) .LT. rden_cut) Then

      ibin = FLOOR(rz(imol,iframe)/bin_wid)
    ...
    Do jmol = imol +1, n_mol_tot
      ...
      DO isite = 1, n_mol_sites
        ...
        DO jsite = 1, n_mol_sites
          ...
          pxx = pxx + xijs * xij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pxy = pxy + xijs * yij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pxz = pxz + xijs * zij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)

          pyx = pyx + yijs * xij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pyy = pyy + yijs * yij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pyz = pyz + yijs * zij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          
          pzx = pzx + zijs * xij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pzy = pzy + zijs * yij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
          pzz = pzz + zijs * zij * f_tot / (rijs * dABS(zij)) * (1.0d0/area)
      ...
      ! Special treatment of PBC (only account for z-direction which is perpendicular to the planar interface)
      If (zij .NE. (rzj -rzi)) Then

        ! -- Assume imol in the central box and ij pair contribue once
        ! == Irving-Kirkwood definition ==
        !Loop through the bins on the POSITIVE z side
        DO ibin= 1, n_bins
          
          ! Calculate intersection with planes ibin 
          solk = (bin_posp(ibin) - rzi)/zij
        ...
        !Loop through the bins on the NEGATIVE z side
        DO ibin= 1, n_bins
          
          ! Calculate intersection with planes ibin 
          solk = (bin_posn(ibin) - rzi)/zij
...
! --------------- z-density ---------------!
! Open file
OPEN(3,FILE='z-density.txt',STATUS='UNKNOWN',ACCESS='SEQUENTIAL',ACTION='WRITE')
...
Write(4,'(A)') 'z Pkin Pxx Pxy Pxz Pyx Pyy Pyz Pzx Pzy Pzz'
